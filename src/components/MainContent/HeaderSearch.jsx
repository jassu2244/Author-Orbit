...
export default function HeaderSearch({ value, onChange, loading }) {
  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-3 border-b border-black/10 pb-4 relative z-20">
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
      ...
      {showSuggestions && (
        <div className="absolute left-0 right-0 top-full z-[100] mt-1 overflow-hidden rounded-md border border-black/10 bg-[#FBF9F1] shadow-xl animate-in slide-in-from-top-2 fade-in duration-200">
          <p className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-muted">
            Popular Authors
          </p>
            ...
            {POPULAR_AUTHORS.map((author) => (
              <li key={author}>
                <button
                  ...
                  className="w-full px-4 py-2.5 text-left text-sm font-medium text-ink transition hover:bg-black/5 focus-visible:bg-black/5"
                >
                  {author}
                </button>
              </li>
